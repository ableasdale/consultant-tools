...
(:------------------------------------------------------------------------------
 ...
 : Minor amendment made in 2011 by Alex Bleasdale (ableasdale@marklogic.com) 
 :  to add the ability to change the appserver default user
 : 
            ...
            <valid-options>
                ...
                <option value="appserver-set">
                    ...
                    <allowed-settings>
                        ...
                        <setting cast="xs:unsignedLong">default-user</setting>
    ...
    return
        if (fn:not(admin:appserver-exists($config, $group-id, $appserver-name))) then (
            ...
            return 
                ...
                else if ($setting eq xs:string("default-user")) then (
		    let $appserver-id := admin:appserver-get-id($config, $group-id, $appserver-name)
                    ...
                    (: calling sec:uid-for-name to resolve the correct uid for the user :)
                    let $_value := xdmp:apply($type-constructor, sec:uid-for-name($value))
