...
(:~
 ...
 : you may fn:not use this file except in compliance with the License.
...
module namespace dates = "http://xqdev.com/dateparser";
...
declare function dates:_tokenParse(
    $i as xs:string
) as xs:dateTime?
{
    (
...
declare function dates:_tokParse1(
    $bits as xs:string*,
    $pos as xs:integer
) as xs:dateTime?
{
...
declare function dates:_tokParse2(
    $bits as xs:string*,
    $pos as xs:integer
) as xs:dateTime?
{
...
declare function dates:_tokParse3(
    $bits as xs:string*,
    $pos as xs:integer
) as xs:dateTime?
{
...
declare function dates:_tokParse4(
    $bits as xs:string*,
    $pos as xs:integer
) as xs:dateTime?
{
...
declare function dates:_dashParse(
    $i as xs:string
) as xs:dateTime?
{
    (: 25-oct-2004 17:06:46 -0500 :)
...
declare function dates:_processYear(
    $year as xs:string?
) as xs:string?
{
    ...
    then
        ...
        then
            ...
            then "1900"
            else concat("20", functx:pad-integer-to-length("0", 2 - string-length($year)), $year)
...
declare function dates:_monthNameToNumber(
    $month as xs:string?
) as xs:string?
{
    (
        ...
        return dates:process-padding(2 - string-length(string($pos)), $pos) 
    )[1]
};

(: 
 : Quickly hacked to make it work with functx lib
 :)
declare function dates:process-padding($a as xs:integer, $b) as xs:string{
    let $c := if ($a eq 0) then (string($b))
    else (concat(functx:pad-integer-to-length("0", $a), $b) )
    return $c
};

declare function dates:_processDay(
    $day as xs:string?
) as xs:string?
{
    dates:process-padding(2 - string-length(string($day)), $day) 
};

declare function dates:_processTime(
    $time as xs:string?
) as xs:string
{
    ...
    let $hours :=
        if(exists($bits[1]))
        then concat(functx:pad-integer-to-length("0", 2 - string-length($bits[1])), $bits[1])
        else "00"
    let $min :=
        if(exists($bits[2]))
        then concat(functx:pad-integer-to-length("0", 2 - string-length($bits[2])), $bits[2])
        else "00"
    let $sec :=
        if(exists($bits[3]))
        then concat(functx:pad-integer-to-length("0", 2 - string-length($bits[3])), $bits[3])
        else "00"
    return string-join(($hours, $min, $sec), ":")
};

declare function dates:_processZone(
    $zone as xs:string?
) as xs:string
{
...
declare function dates:_zoneLookup(
    $s as xs:string
) as xs:string
{
    let $s := upper-case($s)
...
:)
declare function dates:_convertShortcutsToTPlusMinus(
    $date as xs:string
) as xs:string?
{
...
:)
declare function dates:_convertTPlusMinusToDate(
    $date as xs:string
) as xs:string?
{
...
(:
    ...
    into a xs:date.  The date formats can look like:
        ...
        September 1st, 2007
:)
declare function dates:_normalizeDateFormats(
    $date as xs:string
) as element(date)?
{
...
(:
    ...
    is set to "day".
:)
declare function dates:_getDateResolution(
...
:)
declare function dates:_calculateRange(
...
:)
declare function dates:_getLastDayOfTheMonth(
    $year as xs:integer,
    $month as xs:integer
) as xs:date
{
    ...
    let $nextMonth := concat(functx:pad-integer-to-length("0", 2 - string-length($nextMonth)), $nextMonth)
    let $nextDate :=
        xs:date(string-join((
            $nextYear,
            $nextMonth,
...
:)
declare function dates:_expandYear(
    $year as xs:string?
) as element(number)
{
...
(:
    Takes in a 'month' as a string and returns it as a number.  For example:
        ...
        "feb" -> "06"
:)
declare function dates:_expandMonth(
    $month as xs:string?
) as element(number)
{
...
:)
declare function dates:_expandDay(
    $day as xs:string?
) as element(number)
{
...
declare function dates:_expandTwoDigits(
    $num as xs:string?
) as element(number)
{
    (:
    ...
    else <number type="value">{ concat(functx:pad-integer-to-length("0", 2 - string-length($num)), $num) }</number>
};
